...
implicit class RowUpdatesToSudokuField(
    val update: Vector[SudokuDetailProcessor.RowUpdate]
) extends AnyVal {
...
implicit class SudokuFieldOps(val sudokuField: SudokuField) extends AnyVal {
  ...
  def flipHorizontally: SudokuField =
    sudokuField.rotateCW.flipVertically.rotateCCW
  ...
  def randomSwapAround: SudokuField = {
    ...
    val shuffledValuesMap =
      possibleCellValues
        .zip(scala.util.Random.shuffle(possibleCellValues))
        .to(Map) + (0 -> 0)
