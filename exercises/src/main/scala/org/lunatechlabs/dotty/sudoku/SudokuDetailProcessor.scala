...
object SudokuDetailProcessor {
  ...
  def apply[DetailType <: SudokoDetailType](
    ...
    Behaviors.setup { context =>
      (new SudokuDetailProcessor[DetailType](context))
        .operational(id, state, fullyReduced = false)
    }
...
class SudokuDetailProcessor[
    DetailType <: SudokoDetailType: UpdateSender
] private (
    ...
    Behaviors.receiveMessage {
      case Update(cellUpdates, replyTo) if !fullyReduced =>
        ...
        if (updatedState == previousState && cellUpdates != cellUpdatesEmpty) {
          ...
          val transformedUpdatedState =
            updatedState.reductionRuleOne.reductionRuleTwo
  ...
  private def stateChanges(
      state: ReductionSet,
      updatedState: ReductionSet
  ): CellUpdates =
